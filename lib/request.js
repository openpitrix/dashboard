...
const formatErrMsg = (errMsg = '', reqUrl, reqParams) => `${errMsg}\n\nRequest url: ${reqUrl} \nRequest params: ${JSON.stringify(
  reqParams
)}`;
  ...
  static send = async (method, url, params = {}, options = {}) => {
    ...
    try {
      ...
      if (err.response) {
        ...
        return {
          err: formatErrMsg(statusText, reqUrl, params),
          status
        };
      }
      ...
      if (err.message) {
        ...
        return {
          err: formatErrMsg(err.message, reqUrl, params),
      ...
      return {
        err: formatErrMsg('Unknown request error', reqUrl, params),
        status: 500
      };
    }
  };
